...
### Browser Events in React - Review 

Like many things in React, event handling is a little different than what we are used to.

Here are the 2 main differences we have discussed:

### 1. Using Handler Functions for Events in React
...
We can add a `required` and `pattern` attribute to HTML inputs to validate their data. React works with those props too!
